...
| 항목 | 결정 |
|------|------|
| response processing mode | `Buffered` — EnvoyExtensionPolicy `processingMode.request/response.body: Buffered` |
| custom ExtProc 연결 방식 | `EnvoyExtensionPolicy` CRD — GatewayConfig만으로는 연결 불가 (관련 이슈: `docs/issues/2026-04-24-gatewayconfig-extproc-schema.md`) |
| user 메시지 저장 시점 | 요청 mutation 시점 (request_body 단계에서 Redis Append) |
| OpenAI streaming 응답 | **1차 PoC 범위 제외** — SSE streaming은 body buffering이 불가해 별도 처리 필요, 후속 과제로 분류 |
