...
- 이 프로젝트에서는 사용자가 sub-agent 사용을 허용했으므로, 에이전트가 필요하다고 판단하면 별도 재확인 없이 sub-agent를 사용할 수 있습니다.
- 실 구현 작업 위임 시 기본은 `worker` sub-agent를 사용하고, 모델은 `gpt-5.3-codex`를 우선합니다.
- 코드베이스 탐색, 구조 분석, 영향 범위 조사처럼 병렬 처리에 적합한 작업은 `explorer` sub-agent를 사용할 수 있습니다.
- 단일 파일의 작은 수정, 즉시 처리 가능한 문서/설정 변경, 메인 작업의 blocking path에 있는 구현은 메인 에이전트가 직접 수행할 수 있습니다.
- sub-agent에 위임할 때는 담당 파일/모듈 범위를 명확히 지정하고, 다른 변경을 되돌리지 않습니다.
- 최종 통합, 검증, 커밋/push, 사용자 보고는 메인 에이전트가 책임집니다.
...
- 문제, 특이 사항, 검증 실패, 문서/버전 충돌이 발생하면 `docs/issue-log.md`를 기준으로 발생부터 해결까지 기록합니다.

### 문제/특이 사항 기록

- 기록 대상, 저장 위치, 템플릿은 `docs/issue-log.md`를 따릅니다.
- 문제 기록 파일은 `docs/issues/` 아래에 날짜와 짧은 제목을 포함한 Markdown 파일로 저장합니다.
- 해결 전이라도 재발 가능성이 있거나 프로젝트 방향에 영향을 주는 문제는 먼저 기록하고, 해결 후 같은 문서를 업데이트합니다.
- 민감정보가 포함된 오류 출력은 그대로 남기지 않고 필요한 부분만 마스킹하거나 요약합니다.
...
- 커밋 메시지, GitHub Issue, PR 제목과 본문은 기본적으로 한글로 작성합니다.
- 커밋 메시지는 필요하면 `docs:`, `feat:`, `chore:` 같은 conventional prefix를 유지하되, 설명 문장은 한글로 작성합니다.
